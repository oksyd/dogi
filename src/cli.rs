...
const LINUX_UDEV_RULE: &str = include_str!("../packaging/linux/70-dogi-logitech.rules");
...
fn format_hidpp_features(features: &[crate::domain::HidppFeatureInfo]) -> String {
...
fn transaction_status_label(status: crate::domain::SettingsTransactionState) -> &'static str {
    match status {
        crate::domain::SettingsTransactionState::Committed => "committed",
        crate::domain::SettingsTransactionState::Rejected => "rejected before write",
        crate::domain::SettingsTransactionState::RolledBack => "rolled back",
        crate::domain::SettingsTransactionState::RecoveryRequired => "recovery required",
    }
}
...
fn upsert_app_profile(
    ...
    let mut buttons = args
        .button_mappings
        .iter()
        .map(|mapping| crate::domain::ButtonBinding {
    ...
    if !args.gesture_mappings.is_empty() {
            ...
            .find(|binding| binding.button == Master3sButton::Gesture)
        {
            ...
            Some(_) => {}
            None => buttons.push(crate::domain::ButtonBinding {
...
mod tests {
    ...
    fn formats_runtime_actions() {
        assert_eq!(
            format_resolved_runtime_action(&ResolvedRuntimeAction {
                ...
                command: crate::domain::RuntimeCommand::KeyChord(vec![
                    crate::domain::RuntimeKey::Alt,
                    crate::domain::RuntimeKey::Left,
                ]),
            }),
        ...
        assert_eq!(
            format_resolved_runtime_action(&ResolvedRuntimeAction {
                ...
                command: crate::domain::RuntimeCommand::Unsupported,
            }),
        ...
        assert_eq!(
            format_runtime_action_execution(&RuntimeActionExecution {
                action: ResolvedRuntimeAction {
                    ...
                    command: crate::domain::RuntimeCommand::KeyChord(vec![
                        crate::domain::RuntimeKey::Control,
                        crate::domain::RuntimeKey::Equal,
                    ]),
                },
    ...
    fn make_runtime_device(id: &str, hidpp: bool, paired: bool) -> DeviceInfo {
        DeviceInfo {
            ...
            paired_device: paired.then(|| crate::domain::PairedDeviceInfo {
                slot: 1,
            ...
            report_descriptor: crate::domain::ReportDescriptorInfo {
                hidpp_usage: hidpp.then_some(crate::domain::HidUsage {
                    usage_page: 0xff00,
                    usage: 0x0001,
                }),
                ..crate::domain::ReportDescriptorInfo::default()
            },
            capabilities: crate::domain::DeviceCapabilities {
                hidpp: if hidpp {
                    CapabilityState::Supported
                } else {
                    CapabilityState::Unknown
                },
                ..crate::domain::DeviceCapabilities::default()
            },
        }
    }
}
