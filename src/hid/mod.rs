...
mod api_tests {
    use super::*;
    use crate::domain::build_master3s_apply_plan;
    ...
    fn partial_apply_rejects_a_plan_for_another_device_before_io() {
        ...
        let error = prepare_master3s_settings_plan("device-b", &settings, &plan)
...
mod platform {
    ...
    use crate::domain::{
        ...
        HidppFeature, HidppFeatureInfo, HidppProbeIssue, HidppProtocolVersion, Master3sSettings,
        PairedDeviceInfo, ReceiverKind, ReportDescriptorInfo, Result, SettingsApplyOperation,
        SettingsApplyOutcome, SettingsApplyPlan, SettingsApplyReport, SettingsApplyStatus,
        SettingsTransactionState, ThumbWheelMode, WheelRatchetMode, WritePolicy,
        bus_kind_from_linux_bus_id, button_action_requires_runtime, infer_connection,
        infer_receiver_kind, is_logitech_vendor, master3s_button_control_id, stable_device_id,
    };
    ...
    impl Master3sRuntimeEventListener {
        ...
        fn open_device(device: &DeviceInfo) -> Result<Self> {
            ...
            let consumer_lease =
                EndpointConsumerLease::acquire(crate::domain::hidpp_endpoint_id(&device.id))
    ...
    fn read_hidraw_devices(
        ...
        let connection = match bus {
            crate::domain::BusKind::Bluetooth => ConnectionKind::Bluetooth,
        ...
        Ok(hidpp_probes
            .into_iter()
            .map(|probe| {
                let access = DeviceAccess { hidpp_issue: probe.issue, ..access.clone() };
                ...
                let id = probe.paired_device.as_ref().map_or_else(
                    || endpoint_id.clone(),
                    |paired| crate::domain::logical_hidpp_device_id(&endpoint_id, paired),
                );
    ...
    fn prepared_device_identity(
        ...
        Ok(PreparedDeviceIdentity {
            receiver_id: crate::domain::hidpp_endpoint_id(&device.id).to_owned(),
    ...
    fn probe_settings_target(
        ...
        if !paired.features_complete {
            return Err(DogiError::Transport(
                "device feature discovery is incomplete; retry before applying settings".to_owned(),
            ));
        }
    ...
    fn prepare_pointer_speed(
        ...
        super::pointer::prepare(
            features,
            settings.pointer_speed_percent,
    ...
    fn prepare_smart_shift(
        ...
        let (feature_id, index, read_function, write_function) =
            smart_shift_feature_access(features).ok_or_else(|| {
                DogiError::UnsupportedFeature("SMART_SHIFT is not available".to_owned())
            })?;
    ...
    fn prepare_button_diversion(
        ...
        let raw_xy = binding.action == crate::domain::ButtonAction::Gestures;
    ...
    fn require_feature(features: &[HidppFeatureInfo], feature_id: u16, name: &str) -> Result<u8> {
        feature_index_in(features, feature_id)
            .ok_or_else(|| DogiError::UnsupportedFeature(format!("{name} is not available")))
    }
    ...
    fn probe_issue(error: &io::Error) -> HidppProbeIssue {
        if error.kind() == io::ErrorKind::WouldBlock {
            HidppProbeIssue::Busy
        } else {
            HidppProbeIssue::Failed
        }
    }
    ...
    fn decode_feature_index(reply: Option<&[u8]>) -> io::Result<Option<u8>> {
        let reply = reply.ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::TimedOut,
                "feature lookup did not return a reply",
            )
        })?;
        let index = reply.first().copied().ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                "feature lookup returned an empty reply",
            )
        })?;
    ...
    impl HidppClient {
        ...
        fn feature_index(&mut self, devnumber: u8, feature: u16) -> io::Result<Option<u8>> {
            ...
            decode_feature_index(reply.as_deref())
        }
    ...
    mod tests {
        ...
        fn probe_errors_are_distinct_from_no_response() {
            for (kind, expected) in [
                (io::ErrorKind::WouldBlock, HidppProbeIssue::Busy),
                (io::ErrorKind::BrokenPipe, HidppProbeIssue::Failed),
            ] {
                let mut client = FakeProbeClient::new(test_receiver_pairing());
                for _ in 0..=HIDPP_PROBE_RETRY_DELAYS.len() {
                    client
                        .pings
                        .push_back(Err(io::Error::new(kind, "probe failed")));
                }
                let probe = probe_hidpp_device(&mut client, Some(ReceiverKind::Bolt), |_| {});
                assert_eq!(probe.issue, Some(expected));
                assert_eq!(probe.paired_device.unwrap().wpid.as_deref(), Some("B034"));
            }

            let mut client = FakeProbeClient::new(test_receiver_pairing());
            let probe = probe_hidpp_device(&mut client, Some(ReceiverKind::Bolt), |_| {});
            assert_eq!(probe.issue, None);
            assert!(probe.paired_device.is_some());
        }

        #[test]
        fn unanswered_feature_lookup_is_not_an_unsupported_feature() {
            assert_eq!(
                decode_feature_index(None).unwrap_err().kind(),
                io::ErrorKind::TimedOut
            );
            assert_eq!(
                decode_feature_index(Some(&[])).unwrap_err().kind(),
                io::ErrorKind::InvalidData
            );
            assert_eq!(decode_feature_index(Some(&[0, 0, 0])).unwrap(), None);
            assert_eq!(decode_feature_index(Some(&[13, 0, 2])).unwrap(), Some(13));
        }

        #[test]
        fn failed_device_details_preserve_the_known_receiver_slot() {
            let mut client = FakeProbeClient::new(test_receiver_pairing());
            client.pings.push_back(Ok(Some(test_protocol())));
            client.paired_devices.push_back(Err(io::Error::new(
                io::ErrorKind::BrokenPipe,
                "query failed",
            )));
            let probe = probe_hidpp_device(&mut client, Some(ReceiverKind::Bolt), |_| {});
            assert_eq!(probe.issue, Some(HidppProbeIssue::Failed));
            assert_eq!(probe.paired_device.unwrap().wpid.as_deref(), Some("B034"));
        }

        #[test]
        fn incomplete_feature_discovery_is_retryable_not_unsupported() {
            let mut client = FakeProbeClient::new(test_receiver_pairing());
            client.pings.push_back(Ok(Some(test_protocol())));
            let mut paired = test_paired_device(20);
            paired.features_complete = false;
            for _ in 0..=HIDPP_PROBE_RETRY_DELAYS.len() {
                client.paired_devices.push_back(Ok((paired.clone(), None)));
            }
            assert!(matches!(
                probe_settings_target(&mut client, Some(ReceiverKind::Bolt), 1),
        ...
        #[test]
        fn dpi_change_uses_verified_transaction_and_restores_the_original_resolution() {
            let feature = HidppFeatureInfo {
                index: 13,
                feature_id: HIDPP_FEATURE_ADJUSTABLE_DPI,
                name: "ADJUSTABLE_DPI".to_owned(),
                flags: 0,
                version: 2,
            };
            let change = super::super::pointer::prepare(&[feature], 150, |_, function, _| {
                Ok(match function {
                    0x00 => vec![1],
                    0x10 => vec![0, 0, 200, 0xe0, 50, 0x1f, 0x40, 0, 0],
                    0x20 => vec![0, 3, 32, 3, 232], // current 800, default 1000
                    _ => panic!("preparation must only read"),
                })
            })
            .unwrap();
            let mut transaction = test_transaction();
            transaction.changes[0] = change;
            for fail_after_write in [None, Some(2)] {
                let mut io = FakeSettingsIo {
                    fail_after_write,
                    ..Default::default()
                };
                io.states
                    .insert(HIDPP_FEATURE_ADJUSTABLE_DPI, vec![0, 3, 32, 3, 232]);
                io.states.insert(0x2121, vec![20]);
                let report = execute_prepared_transaction(&mut io, &transaction).unwrap();
                let expected = if fail_after_write.is_some() {
                    assert_eq!(report.transaction, SettingsTransactionState::RolledBack);
                    vec![0, 3, 32]
                } else {
                    assert_eq!(report.transaction, SettingsTransactionState::Committed);
                    vec![0, 5, 220] // 1500 DPI
                };
                assert_eq!(io.states[&HIDPP_FEATURE_ADJUSTABLE_DPI], expected);
                assert!(
                    io.writes
                        .iter()
                        .all(|(id, _)| *id != HIDPP_FEATURE_POINTER_SPEED)
                );
            }
        }
...
mod platform {
    ...
    use super::{Master3sRuntimeEvent, PreparedSettingsTransaction};
