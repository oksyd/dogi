...
fn ensure_private_directory_platform(
    ...
    for component in components {
        ...
        if fd < 0 {
            let source = io::Error::last_os_error();
            let source = if matches!(source.raw_os_error(), Some(libc::ENOTDIR | libc::ELOOP)) {
                io::Error::new(
                    source.kind(),
                    format!("{source}; use a real directory, not a symbolic link"),
                )
            } else {
                source
            };
...
mod tests {
    ...
    fn private_directory_creation_rejects_symlink_components() {
        ...
        assert!(error.to_string().contains("use a real directory"));
        assert!(
            fs::symlink_metadata(root.join("redirect"))
                .unwrap()
                .is_symlink()
        );
