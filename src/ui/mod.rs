    ...
    if discovery.is_some() {
        rescan_timer.start(
            ...
            move || {
                ...
                let next_devices = self::logical_devices(&scanned);
...
fn devices_need_hidpp_recovery(devices: &[LogicalDevice]) -> bool {
    devices.iter().any(|device| {
        device.hidpp_count > 0
            ...
            && (logical_device_is_master3s(device) || device.primary.access.hidpp_issue.is_some())
...
fn logical_device_name(device: &LogicalDevice) -> String {
    ...
    if is_receiver_endpoint(&device.primary) {
        match device.primary.receiver_kind {
            Some(crate::domain::ReceiverKind::Bolt) => "Logi Bolt receiver".to_owned(),
            Some(crate::domain::ReceiverKind::Unifying) => "Logitech Unifying receiver".to_owned(),
            Some(crate::domain::ReceiverKind::Lightspeed) => {
                "Logitech LIGHTSPEED receiver".to_owned()
            }
...
fn logical_device_access_state(device: &LogicalDevice) -> DeviceAccessState {
    ...
    if let Some(issue) = device.primary.access.hidpp_issue {
        return match issue {
            crate::domain::HidppProbeIssue::Busy => DeviceAccessState::ProbeBusy,
            crate::domain::HidppProbeIssue::Failed => DeviceAccessState::ProbeFailed,
        };
    }
...
fn logical_device_is_ready(device: &LogicalDevice) -> bool {
    device.hidpp_count > 0
        && device.primary.access.hidraw_readwrite
        && device.primary.access.hidpp_issue.is_none()
...
fn plan_row_from_step(step: &SettingsApplyStep, settings: &Master3sSettings) -> PlanRow {
    ...
    match &step.operation {
        ...
        SettingsApplyOperation::ButtonMapping { button, action } => {
            ...
            row.enabled = crate::domain::button_action_requires_runtime(*button, *action);
        }
...
fn device_plan_rows_from_plan(
    plan: &crate::domain::SettingsApplyPlan,
    settings: &Master3sSettings,
) -> Vec<PlanRow> {
    plan.steps
        .iter()
    ...
    preview
        ...
        .map(|step| {
            ...
            match (&step.before, &step.after) {
                (
                    DeviceSettingValue::PointerDpi { dpi: before },
                    DeviceSettingValue::PointerDpi { dpi: after },
                ) => {
                    row.kind = PlanKind::PointerDpi;
                    row.before_value = i32::from(*before);
                    row.value = i32::from(*after);
                }
                (
...
mod tests {
    ...
    fn schedules_recovery_only_for_an_accessible_master3s_without_battery() {
        ...
        missing_battery.battery = crate::domain::BatteryInfo {
            level_percent: Some(80),
            status: BatteryStatus::Discharging,
            source: crate::domain::BatterySource::Hidpp,
            detail: None,
        };
        let logical = logical_devices(&[missing_battery.clone()]);
        assert!(!devices_need_hidpp_recovery(&logical));
        ...
        missing_battery.battery =
            crate::domain::BatteryInfo::not_queried("battery requires HID++ query");
    ...
    #[test]
    fn probe_issues_are_not_reported_as_unresponsive_mice() {
        for (issue, expected) in [
            (
                crate::domain::HidppProbeIssue::Busy,
                DeviceAccessState::ProbeBusy,
            ),
            (
                crate::domain::HidppProbeIssue::Failed,
                DeviceAccessState::ProbeFailed,
            ),
        ] {
            ...
            endpoint.access.hidpp_issue = Some(issue);
            let logical = logical_devices(&[endpoint.clone()]);
            assert_eq!(device_row_from_logical(&logical[0]).access_state, expected);
            assert!(devices_need_hidpp_recovery(&logical));
            assert!(!logical_device_is_ready(&logical[0]));

            endpoint.access.hidraw_readwrite = false;
            let logical = logical_devices(&[endpoint]);
            assert_eq!(
                device_row_from_logical(&logical[0]).access_state,
                DeviceAccessState::ReadOnly
            );
            assert!(!devices_need_hidpp_recovery(&logical));
        }
    }
    ...
    #[test]
    fn dpi_preview_displays_actual_resolution_instead_of_percentage() {
        let settings = Master3sSettings {
            pointer_speed_percent: 125,
            ..Default::default()
        };
        let preview = SettingsApplyPreview {
            device_id: "mouse".to_owned(),
            profile_name: settings.profile_name.clone(),
            steps: vec![crate::domain::SettingsApplyPreviewStep {
                operation: SettingsApplyOperation::PointerSpeed { percent: 125 },
                feature: crate::domain::HidppFeature::PointerSpeed,
                before: DeviceSettingValue::PointerDpi { dpi: 800 },
                after: DeviceSettingValue::PointerDpi { dpi: 1250 },
            }],
        };
        let rows = device_plan_rows_from_preview(&preview, &settings);
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].kind, PlanKind::PointerDpi);
        assert!(rows[0].has_before);
        assert_eq!(rows[0].before_value, 800);
        assert_eq!(rows[0].value, 1250);
    }
    ...
    fn logical_device_reports_missing_hidraw_node_separately_from_permissions() {
        ...
        endpoint.battery = crate::domain::BatteryInfo::not_queried(
    ...
    fn device_session_keeps_receiver_slots_separate_during_identity_promotion() {
        ...
        for device in [&mut first, &mut second] {
            device.id = crate::domain::logical_hidpp_device_id(
                &device.id,
    ...
    fn apply_report_counts_remain_structured_for_translation() {
        let report = SettingsApplyReport {
            ...
            transaction: crate::domain::SettingsTransactionState::Committed,
            outcomes: vec![
    ...
    fn renders_preview() {
        ...
        let access_preview = std::env::var("DOGI_UI_SNAPSHOT_ACCESS")
            ...
            .map(|state| match state.as_str() {
                ...
                "busy" => DeviceAccessState::ProbeBusy,
                "query-failed" => DeviceAccessState::ProbeFailed,
    ...
    fn make_device(path: &str, physical_path: &str, hidpp: CapabilityState) -> DeviceInfo {
        DeviceInfo {
            ...
            battery: crate::domain::BatteryInfo::not_queried("battery requires HID++ query"),
